...
public class KeioSagamiharaLine extends LineData {
    ...
    protected String getStationDataCsvPath() {
        return "time_table/keio_sagamihara_line_station.csv";
    }

    @Override
    protected String getTimeTableOutCsvPath() {
        return "time_table/keio_sagamihara_line_weekdays_out.csv";
    }

    @Override
    protected String getTimeTableInCsvPath() {
        return "time_table/keio_sagamihara_line_weekdays_in.csv";
    }
    ...
    public Point calcPositionOnLinePath(float dist, Direction direction) {
        ...
        EasyPathPoint[] epp = {
                // 調布 - 橋本
                LineSegmentPath.getInstance(0.3f,
                        new Point(origin.x + 1000, origin.y + offset),
                        new Point(origin.x + 800, 200 + origin.y + offset)),
                LineSegmentPath.getInstance(1.0f,
                        new Point(origin.x + 800, 200 + origin.y + offset),
                        new Point(origin.x + 200, 200 + origin.y + offset)),
                // 終わり
                LineSegmentPath.getInstance(Float.MAX_VALUE,
                        new Point(origin.x + 200, origin.y + 200 + offset),
                        new Point(origin.x + 200, origin.y + 200 + offset))
        };
